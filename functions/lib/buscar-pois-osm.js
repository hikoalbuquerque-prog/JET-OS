...
function distMetros(la1, ln1, la2, ln2) {
    const R = 6371000;
...
function parseOverpassElements(elements, refLat, refLng) {
    const seen = new Set();
    const result = [];
    for (const el of elements) {
        ...
        // Determinar tipo
        const allVals = [
            ...Object.values(tags).flatMap((v) => String(v).split(';').map((s) => s.trim())),
        ];
        const tipo = allVals.reduce((t, v) => t || TIPO_MAP[v] || '', '') || 'outro';
        result.push({
            id: `${el.type}-${el.id}`,
            tipo,
            nome,
            ...
            tags: Object.fromEntries(Object.entries(tags).slice(0, 10).map(([k, v]) => [k, String(v)])),
        });
    }
...
// ─── Callable principal ───────────────────────────────────────────────────────
exports.buscarPOIsOSMFn = functions.https.onCall({
    ...
    if (!lat || !lng)
        return { ok: false, erro: 'lat/lng obrigatórios', pois: [] };
    let allPois = [];
    if (useGrid && raio > 3) {
...
──────────────────────────────────────────────────────────────────────────────
*/
//# sourceMappingURL=buscar-pois-osm.js.map