...
exports.espelharOcorrenciaSupabase = void 0;
...
async function resolverUuidSupabase(firebaseUid) {
    if (!firebaseUid)
        return null;
...
exports.espelharOcorrenciaSupabase = (0, firestore_1.onDocumentWritten)({ document: 'ocorrencias/{id}', region: 'southamerica-east1', maxInstances: 10 }, async (event) => {
    ...
    if (!event.data?.after?.exists) {
        try {
            await fetch(`${SUPABASE_URL}/rest/v1/ocorrencias?firebase_doc_id=eq.${encodeURIComponent(firebaseDocId)}`, { method: 'DELETE', headers: hdr });
        }
        catch (e) {
            functions.logger.error('[mirror-ocor] delete:', e);
        }
        return;
    }
    ...
    try {
        ...
        const resp = await fetch(`${SUPABASE_URL}/rest/v1/ocorrencias?on_conflict=firebase_doc_id`, {
            method: 'POST',
            headers: {
                apikey: SUPABASE_SERVICE_ROLE,
                Authorization: `Bearer ${SUPABASE_SERVICE_ROLE}`,
...
//# sourceMappingURL=mirror-ocorrencias.js.map