...
// ─── helpers de data ─────────────────────────────────────────────────
function getDataOcorrencia(o) {
    if (o.criadoEm?.toDate)
        return o.criadoEm.toDate();
    if (o.created_at) {
...
async function buscarOcorrencias(ini, fim) {
    const rows = await (0, supabase_rest_1.supabaseGet)('ocorrencias', `select=*&created_at=gte.${encodeURIComponent(ini.toISOString())}&created_at=lte.${encodeURIComponent(fim.toISOString())}`);
    if (!rows)
        return [];
    return rows.map((r) => ({
        id: r.id,
...
async function getTelegramConfig() {
    ...
    const supa = await (0, config_supabase_1.getAppSetting)('config_telegram');
    if (supa) {
    ...
    const supaLegacy = await (0, config_supabase_1.getAppSetting)('telegram');
    if (supaLegacy) {
    ...
    throw new Error('Config Telegram não encontrada no Supabase (app_settings/config_telegram ou telegram_config). ' +
...
function gerarTextoGuard(ocorrs, label, semanal) {
    ...
    if (procurand > 0)
        txt += `└ ‼️ PROCURADOS: *${procurand}* ativo(s) em aberto!\n`;
    else
        txt += `└ Fora de zona: *${fora}*\n`;
    txt += `\n`;
    // Por tipo com barras
    if (tiposOrd.length) {
        txt += `*📍 Por tipo*\n`;
    ...
    if (cidadesOrd.length) {
        ...
        cidadesOrd.slice(0, 8).forEach(([cidade, v]) => {
            ...
            const parts = [];
            if (v.roubos)
                parts.push(`🔴 ${v.roubos} roubos`);
            if (v.recup)
                parts.push(`✅ ${v.recup} recup.`);
            if (v.abertos)
                parts.push(`🔓 ${v.abertos} abertos`);
            if (v.criticos)
                parts.push(`🚨 ${v.criticos} crít.`);
            if (v.procurando)
                parts.push(`‼️ ${v.procurando} proc.`);
            if (parts.length)
                txt += `  ${parts.join('  ')}\n`;
            // Ativos por tipo
    ...
    const alertas = [];
    if (procurand > 0)
        alertas.push(`‼️ *${procurand} ativo(s) PROCURADO(S)* — requer ação imediata!`);
    if (criticos > 0)
        alertas.push(`🚨 ${criticos} ocorrência(s) de alta prioridade`);
    if (abertos + apuracao > 0)
        alertas.push(`⏳ ${abertos + apuracao} pendente(s) de resolução`);
    if (total > 0 && fora / total > 0.25)
        alertas.push(`📍 ${Math.round(fora / total * 100)}% fora de zona cadastrada`);
    if (tipoDom && tipoDom[1] >= 2)
        alertas.push(`📌 Tipo dominante: *${tipoDom[0]}* (${tipoDom[1]}x)`);
    if (topCidade && topCidade[1].total >= 2)
        alertas.push(`🔺 Concentração em *${topCidade[0]}* (${topCidade[1].total} ocorr.)`);
    if (alertas.length) {
        txt += `${'─'.repeat(28)}\n`;
        txt += `*⚠️ Alertas*\n`;
...
function gerarSvgBarras(dados, titulo, W = 480, H = 160) {
...
function gerarSvgPizza(dados, titulo, W = 220, H = 160) {
...
function gerarHtmlPdf(ocorrs, titulo, label, tipo, ocorrsAcum) {
    const agora = new Date();
    const ontemDt = new Date(agora);
    ...
    ocorrs.forEach(o => {
        const c = o.cidade_inicial || 'Desconhecida';
        if (!porCidade[c])
            porCidade[c] = { total: 0, criticos: 0, abertos: 0, recup: 0, fora: 0 };
        porCidade[c].total++;
        if (o.prioridade === 'Alta' || o.prioridade === 'Critica')
            porCidade[c].criticos++;
        if (o.status === 'Aberto')
            porCidade[c].abertos++;
        if (/recuper/i.test(o.status || ''))
            porCidade[c].recup++;
    ...
    const alertas = [];
    if (procurand > 0)
        alertas.push(`‼️ <b>${procurand} ativo(s) PROCURADO(S)</b> — requer ação imediata!`);
    if (criticos > 0)
        alertas.push(`🚨 ${criticos} ocorrência(s) de alta prioridade`);
    if (abertos + apuracao > 0)
        alertas.push(`⏳ ${abertos + apuracao} pendente(s) de resolução`);
    if (roubos > 0)
        alertas.push(`🔴 ${roubos} roubo(s) registrado(s)`);
    if (ocorrs.length > 0 && cidadesOrd[0]?.[1].total / ocorrs.length > 0.5)
        alertas.push(`📍 Concentração: mais de 50% em ${cidadesOrd[0][0]}`);
    // Linhas da tabela de cidades
    const cityRows = cidadesOrd.map(([c, v]) => `<tr><td><b>${c}</b></td><td class="n">${v.total}</td><td class="n r">${v.abertos}</td>` +
        `<td class="n g">${v.recup}</td><td class="n c">${v.criticos}</td><td class="n">${v.fora}</td></tr>`).join('');
    // Ocorrências detalhadas agrupadas por cidade
    const porCidadeDetalhe = {};
    ocorrs.forEach(o => {
        const c = o.cidade_inicial || 'Desconhecida';
        if (!porCidadeDetalhe[c])
            porCidadeDetalhe[c] = [];
    ...
    Object.entries(porCidadeDetalhe).sort((a, b) => b[1].length - a[1].length).forEach(([cidade, ocs]) => {
        ...
        ocs.forEach(o => {
            ...
            if (isCrit)
                detalhesHtml += `<span class="badge crit-badge">CRÍTICO</span>`;
            if (isProc)
                detalhesHtml += `<span class="badge proc-badge">‼️ PROCURADO</span>`;
            detalhesHtml += `<span class="ocorr-data">${dt ? dt.toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' }) : ''}</span>`;
            detalhesHtml += `</div>`;
            const infos = [];
            if (o.asset_id || o.ativo_tipo)
                infos.push(`Ativo: <b>${[o.ativo_tipo, o.asset_id].filter(Boolean).join(' ')}</b>`);
            if (o.responsavel)
                infos.push(`Resp.: <b>${o.responsavel}</b>`);
            if (o.bairro_inicial)
                infos.push(`Bairro: ${o.bairro_inicial}`);
            if (o.endereco_inicial)
                infos.push(`Endereço: ${o.endereco_inicial}`);
            if (infos.length)
                detalhesHtml += `<div class="ocorr-info">${infos.join('  •  ')}</div>`;
            if (o.descricao)
                detalhesHtml += `<div class="ocorr-desc">${o.descricao}</div>`;
            if (isProc && o.procurando && typeof o.procurando === 'string') {
                detalhesHtml += `<div class="ocorr-proc">🔍 Procurando: ${o.procurando}</div>`;
            }
            // Fotos
            const fotos = [o.foto1_url, o.foto2_url].filter((u) => {
                if (!u)
                    return false;
...
async function enviarTexto(token, chatId, texto) {
    ...
    await new Promise((resolve, reject) => {
        const req = https.request({ hostname: 'api.telegram.org', path: `/bot${token}/sendMessage`, method: 'POST',
            headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(body) } }, res => { res.resume(); res.statusCode === 200 ? resolve() : reject(new Error(`TG ${res.statusCode}`)); });
        req.on('error', reject);
        req.write(body);
        req.end();
    });
}
async function enviarDocumento(token, chatId, htmlContent, filename, caption) {
    ...
    await new Promise((resolve, reject) => {
        const req = https.request({ hostname: 'api.telegram.org', path: `/bot${token}/sendDocument`, method: 'POST',
            headers: form.getHeaders() }, res => { res.resume(); res.statusCode === 200 ? resolve() : reject(new Error(`TG doc ${res.statusCode}`)); });
        req.on('error', reject);
        form.pipe(req);
    });
}
...
exports.enviarRelatorioManual = (0, https_1.onCall)({
    timeoutSeconds: 120,
    memory: '256MiB',
...
exports.relatorioGuardSemanal = (0, scheduler_1.onSchedule)({ schedule: '0 7 * * 1', timeZone: 'America/Sao_Paulo', memory: '512MiB', timeoutSeconds: 300, maxInstances: 10 }, async () => {
    ...
    const sabPassado = new Date(agora);
    sabPassado.setDate(agora.getDate() - 2); // sáb
    const domPassado = new Date(agora);
    domPassado.setDate(agora.getDate() - 8); // dom
    domPassado.setHours(0, 0, 0, 0);
    sabPassado.setHours(23, 59, 59, 999);
    const fmtDate = (d) => d.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit', timeZone: 'America/Sao_Paulo' });
    ...
    const relatorio = await (0, relatorio_1.gerarRelatorioGuard)(domPassado.toISOString().slice(0, 10));
    ...
    for (const o of ocorrs) {
        ...
        if (o.prioridade === 'Alta' || o.prioridade === 'Critica')
            relatorioSemanal.altaPrioridade++;
        if (o.bo_numero)
            relatorioSemanal.comBO++;
    }
