...
exports.analisarCalcadaIA = analisarCalcadaIA;
// src/ia/index.ts
...
function normalizarResultado(raw, totalFrames) {
    const r = {
    ...
    if (r.confianca === 'baixa')
        r.aprovado = false;
    if (r.larguraEstimada.toLowerCase().includes('indefinido'))
        r.aprovado = false;
    if (r.score < 26)
        r.aprovado = false;
    if (r.imagensBloqueantes >= 2)
        r.aprovado = false;
    if (r.imagensPositivas < 1)
        r.aprovado = false;
    return r;
}
function extrairJson(texto) {
    ...
    if (inicio < 0)
        throw new Error('Nenhum JSON encontrado na resposta Gemini.');
    let depth = 0, fim = -1;
    ...
    if (fim < 0)
        throw new Error('JSON incompleto na resposta Gemini (MAX_TOKENS?).');
    return JSON.parse(t.slice(inicio, fim + 1));
}
// ── CHAMADA GEMINI ───────────────────────────────────────────────
async function chamarGemini(parts) {
    const resp = await axios_1.default.post(GEMINI_URL, {
