...
async function aprovarSolicitacao(solicitacaoId, uid, email, roleOverride, senhaTemporaria) {
    ...
    // senhaTemporaria: se fornecida, define como senha inicial (passada como 5º argumento)
    let userRecord;
    try {
        userRecord = await admin.auth().getUserByEmail(sol.email);
        // Atualizar senha se fornecida
        if (senhaTemporaria) {
            await admin.auth().updateUser(userRecord.uid, { password: senhaTemporaria });
        }
    }
    catch {
        userRecord = await admin.auth().createUser({
            email: sol.email,
            password: senhaTemporaria || (Math.random().toString(36).slice(-10) + 'A1!'),
    ...
    // Envia email de acesso para o novo usuário
    try {
        if (senhaTemporaria) {
            // Usuário recebeu senha temporária via WhatsApp — enviar email de boas-vindas com link de reset
            const link = await admin.auth().generatePasswordResetLink(sol.email, {
                url: 'https://jet-os-7.web.app'
            });
            const axios = (await Promise.resolve().then(() => __importStar(require('axios')))).default;
            const botToken = process.env.TELEGRAM_BOT_TOKEN || '';
            // Se tiver bot Telegram configurado, notificar admin
            if (botToken) {
                console.log('[aprovar] Link de reset para ' + sol.email + ': ' + link);
            }
            // Enviar email via Firebase Auth Action Email (usa template configurado no console)
            const FIREBASE_WEB_API_KEY = process.env.FIREBASE_WEB_API_KEY || '';
            if (FIREBASE_WEB_API_KEY) {
                await axios.post('https://identitytoolkit.googleapis.com/v1/accounts:sendOobCode?key=' + FIREBASE_WEB_API_KEY, { requestType: 'PASSWORD_RESET', email: sol.email, continueUrl: 'https://jet-os-7.web.app' });
                console.log('[aprovar] Email de reset enviado para ' + sol.email);
            }
        }
        else {
            // Sem senha temporária — gerar link e enviar
            const FIREBASE_WEB_API_KEY = process.env.FIREBASE_WEB_API_KEY || '';
            const axios = (await Promise.resolve().then(() => __importStar(require('axios')))).default;
            if (FIREBASE_WEB_API_KEY) {
                await axios.post('https://identitytoolkit.googleapis.com/v1/accounts:sendOobCode?key=' + FIREBASE_WEB_API_KEY, { requestType: 'PASSWORD_RESET', email: sol.email, continueUrl: 'https://jet-os-7.web.app' });
                ...
                const link = await admin.auth().generatePasswordResetLink(sol.email, { url: 'https://jet-os-7.web.app' });
                console.log('[aprovar] Link de reset para ' + sol.email + ': ' + link);
            }
        }
    }
...
//# sourceMappingURL=auth_index.js.map