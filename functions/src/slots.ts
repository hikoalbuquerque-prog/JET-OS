...
async function notificarGestoresCidade(
  ...
  for (const g of gestores) {
    const user = await supabaseGetOne<{ telegram_chat_id?: string }>('usuarios', `select=telegram_chat_id&firebase_uid=eq.${encodeURIComponent(g.uid)}`);
...
export const aceitarSlot = onCall(async (request) => {
    ...
    const userData = await supabaseGetOne<any>('usuarios', `select=*&id=eq.${encodeURIComponent(uid)}`);
    if (!userData) {
    ...
    let slotData: any;
    try {
      slotData = await supabaseGetOne<any>('slots', `select=*&id=eq.${encodeURIComponent(slotId)}`);
      if (!slotData) {
      ...
      if (slotData.cargo !== userData.cargo_prestador) {
        throw new HttpsError(
          'permission-denied',
          `Este slot é para ${slotData.cargo}, seu cargo é ${userData.cargo_prestador}`
        );
      }
