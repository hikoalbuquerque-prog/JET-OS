...
export async function gerarRelatorioGuard(dataStr?: string): Promise<RelatorioGuard> {
  ...
  try {
    const sbRows = await supabaseGet<any>('ocorrencias',
      `select=*&criado_em=gte.${dataRef.toISOString()}&criado_em=lte.${dataFim.toISOString()}&order=criado_em.desc`
    );
...
function labelAcumulado(ocs: OcorrenciaRaw[]): string {
  ...
  const anos = ocs.map(o => {
    const d = o.criadoEm?.toDate?.() ?? new Date(o.criadoEm as any);
...
function formatarMensagem(lang: string, r: RelatorioGuard, ocs24h: OcorrenciaRaw[], ocs7d: OcorrenciaRaw[]): string {
...
function buildMensagem(lang: string, r: RelatorioGuard, ocs24h: OcorrenciaRaw[], ocs7d: OcorrenciaRaw[]): string {
  ...
  const acumFmt  = labelAcumulado(r.ocorrencias as OcorrenciaRaw[]);

  const total   = r.totalOcorrencias;
  const ocs     = r.ocorrencias as OcorrenciaRaw[];
  ...
  ocs.forEach(o => {
    ...
    if (!guardsCont[nome]) guardsCont[nome] = { n:0, cargo: o.cargo || (o as any).role || '' };
    guardsCont[nome].n++;
  });
...
async function gerarPdfHtml(
  ...
  const procurand = (r.ocorrencias as any[]).filter(o => !!o.procurando).length;
  const fora    = (r.ocorrencias as any[]).filter(o => !o.lat_inicial).length;
  ...
  const ocorrs    = r.ocorrencias as OcorrenciaRaw[];

  // Danos vandalismo
  const vandComDano   = ocorrs.filter(o => o.tipo === 'Vandalismo' && (o as any).danoValor > 0).length;
  const danoValorTotal= ocorrs.filter(o => o.tipo === 'Vandalismo')
    .reduce((s: number, o: any) => s + (o.danoValor || 0), 0);
  const danoPctMedio  = (() => {
    const com = ocorrs.filter(o => o.tipo === 'Vandalismo' && (o as any).danoPct > 0);
    return com.length > 0
      ? Math.round(com.reduce((s: number, o: any) => s + (o.danoPct || 0), 0) / com.length) : 0;
  })();
  ...
  const porCidadeMap: Record<string, {
    ...
    abertos:number; crit:number; fora:number; ocorrs:any[];
  }> = {};
  (r.ocorrencias as any[]).forEach(o => {
  ...
  const vandRows24h = vand24h.sort((a,b) => {
    ...
    return `<tr><td>${fmtDt(o.criadoEm)}</td>
      ...
      <td>${o.registradoPorNome||'—'}</td>
      <td>${o.bo_numero||'—'}</td>
      <td>${(o as any).danoValor>0?`R$${(o as any).danoValor}`:(o as any).danoPct>0?`${(o as any).danoPct}%`:'—'}${btnFotos}</td></tr>`;
  }).join('');
  ...
  const chartData = (() => {
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      dayR.push(ocsAcum.filter(o => { const t = o.criadoEm?.toDate?.() ?? new Date(o.criadoEm as any); return o.tipo==='Roubo' && t>=d && t<next; }).length);
      dayT.push(ocsAcum.filter(o => { const t = o.criadoEm?.toDate?.() ?? new Date(o.criadoEm as any); return o.tipo==='Tentativa' && t>=d && t<next; }).length);
      dayV.push(ocsAcum.filter(o => { const t = o.criadoEm?.toDate?.() ?? new Date(o.criadoEm as any); return o.tipo==='Vandalismo' && t>=d && t<next; }).length);
    }
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      wkR.push(ocsAcum.filter(o => { const t = o.criadoEm?.toDate?.() ?? new Date(o.criadoEm as any); return o.tipo==='Roubo' && t>=ini && t<fim; }).length);
      wkT.push(ocsAcum.filter(o => { const t = o.criadoEm?.toDate?.() ?? new Date(o.criadoEm as any); return o.tipo==='Tentativa' && t>=ini && t<fim; }).length);
      wkV.push(ocsAcum.filter(o => { const t = o.criadoEm?.toDate?.() ?? new Date(o.criadoEm as any); return o.tipo==='Vandalismo' && t>=ini && t<fim; }).length);
    }
    ...
    for (let m = 0; m < 12; m++) {
      ...
      moR.push(ocsAcum.filter(o => { const t = o.criadoEm?.toDate?.() ?? new Date(o.criadoEm as any); return o.tipo==='Roubo' && t>=ini && t<fim; }).length);
      moT.push(ocsAcum.filter(o => { const t = o.criadoEm?.toDate?.() ?? new Date(o.criadoEm as any); return o.tipo==='Tentativa' && t>=ini && t<fim; }).length);
      moV.push(ocsAcum.filter(o => { const t = o.criadoEm?.toDate?.() ?? new Date(o.criadoEm as any); return o.tipo==='Vandalismo' && t>=ini && t<fim; }).length);
    }
  ...
  cidadesOrd.forEach(([cidade, v]) => {
    ...
    v.ocorrs.sort((a:any,b:any) => {
      ...
      const sigla = siglaResponsavel(o.cargo || (o as any).role || '');
      const infos: string[] = [];
      if (o.ativo_tipo||o.asset_id) infos.push(`Ativo: <b>${[o.ativo_tipo,o.asset_id].filter(Boolean).join(' ')}</b>`);
      if (o.registradoPorNome) infos.push(`Guard: <b>${o.registradoPorNome}</b> <span style="background:#1a1a2e;color:#fff;padding:1px 5px;border-radius:4px;font-size:8px;font-weight:700">${sigla}</span>`);
...
${(() => {
  ...
  for (let i = semanas-1; i >= 0; i--) {
    ...
    const semOcs = (r.ocorrencias as any[]).filter(o => {
      const d = o.criadoEm?.toDate?.() ?? new Date(o.criadoEm as any);
      return d >= ini && d < fim;
    });
...
<div class="section">
  ...
  <table>
    ...
    <tbody>${cidadesOrd.map(([c,v]) =>
      ...
      <td class="n" style="color:#e67e22">${ocorrs.filter((o:any)=>o.tipo==='Tentativa'&&o.cidade_inicial===c).length}</td>
...
export async function enviarRelatorioTelegram(r: RelatorioGuard, lang: string = 'pt'): Promise<void> {
  ...
  const mapSbRows = (rows: any[]): OcorrenciaRaw[] =>
    rows.map(r => ({
      ...r,
      id: r.id ?? r.uid,
      criadoEm: r.criado_em ? new Date(r.criado_em) : null,
  ...
  // Helper: Supabase-first query for ocorrencias >= date
  async function queryOcorrencias(desde: Date): Promise<OcorrenciaRaw[]> {
    try {
      const sbRows = await supabaseGet<any>('ocorrencias',
  ...
  const [ocs24h, ocs7d, ocsMes, ocsAcum] = await Promise.all([
    queryOcorrencias(ini24),
    queryOcorrencias(ini7d),
    queryOcorrencias(iniMes),
    queryOcorrencias(iniAno),
  ]);
  ...
  try {
    const resp = await axios.post(
      ...
      { chat_id: chatId, text: mensagem, parse_mode: 'Markdown', disable_web_page_preview: true },
      { timeout: 15000 }
    );
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      const req = https.request(
        { hostname:'api.telegram.org', path:`/bot${token}/sendDocument`, method:'POST', headers: form.getHeaders() },
