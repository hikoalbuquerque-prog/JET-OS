...
// ─── tipos ───────────────────────────────────────────────────────────
export interface POISalvo {
  id: string;                  // place_id do Google
  fonte: 'google' | 'osm';
  nome: string;
  tipo: string;                // categoria normalizada
  tipos_google: string[];      // types[] originais do Places
  lat: number;
  lng: number;
  endereco: string;
  cidade: string;
  ...
  foto_ref?: string;           // photo_reference do Places
  foto_url?: string;           // URL gerada (salva para evitar chamadas extras)
  street_view_url?: string;    // Static Street View gerado
  maps_url: string;            // link google.com/maps
  salvoPor: string;            // uid de quem disparou
  salvoEm: admin.firestore.Timestamp;
  cidade_busca: string;        // cidade/lat-lng usada na busca original
}
...
function normalizarTipo(types: string[]): string {
  for (const t of types) {
    if (TIPO_MAP[t] && TIPO_MAP[t] !== 'outros') return TIPO_MAP[t];
  }
  return 'outros';
}

// ─── gerar URL da foto Places ─────────────────────────────────────────
function fotoUrl(photoRef: string, apiKey: string, maxWidth = 400): string {
...
async function buscarNearbySearch(
  lat: number, lng: number, raioM: number, tipo: string | undefined, apiKey: string
): Promise<any[]> {
  ...
  else params.keyword = 'ponto de interesse';
...
export async function buscarPOIsOSMInterno(
  lat: number, lng: number, raioM: number
): Promise<any[]> {
...
export async function buscarPOIsGoogleInterno(
  lat: number, lng: number, raioM: number, tipoFiltro?: string
): Promise<any[]> {
  ...
  const buscas = await Promise.allSettled(
    TIPOS.map(t => buscarNearbySearch(lat, lng, raioM, t, apiKey))
  );
...
export const buscarSalvarPOIsGoogle = onCall(
  ...
  async (r: any) => {
    ...
    const pontosGrid: Array<{lat: number; lng: number}> = [];
    if (raioM > 5000) {
    ...
    for (const ponto of pontosGrid) {
      const buscas = await Promise.allSettled(
        TIPOS_PLACES.map(t => buscarNearbySearch(ponto.lat, ponto.lng, raioGrid, t, apiKey))
      );
      buscas.forEach(r => {
        if (r.status === 'fulfilled') {
          r.value.forEach((p: any) => {
            if (p.place_id && !seenIds.has(p.place_id)) {
              seenIds.add(p.place_id); results.push(p);
            }
          });
        }
      });
    ...
    const poisSalvos: POISalvo[] = [];

    for (const place of results) {
      const placeId = place.place_id;
      if (!placeId) continue;

      const tipos: string[] = place.types || [];
      const tipoNorm = normalizarTipo(tipos);
      if (tipoNorm === 'outros' && !tipo) continue; // filtrar genéricos
      ...
      const poiRaw: Record<string, any> = {
        id:           placeId,
        fonte:        'google',
        nome:         place.name || '',
        tipo:         tipoNorm,
      ...
      if (place.rating !== undefined)               poiRaw.rating         = place.rating;
      if (place.user_ratings_total !== undefined)   poiRaw.total_ratings  = place.user_ratings_total;
      if (place.opening_hours?.open_now !== undefined) poiRaw.aberto_agora = place.opening_hours.open_now;
      if (photoRef)                                 poiRaw.foto_ref       = photoRef;
      if (photoRef)                                 poiRaw.foto_url       = fotoUrl(photoRef, apiKey);

      const poi = poiRaw as POISalvo;
...
export const carregarPOIsSalvos = onCall(
  { timeoutSeconds: 30, memory: '256MiB' },
  async (r: any) => {
    if (!r.auth) throw new HttpsError('unauthenticated', 'Auth necessária');

    const { lat, lng, raioM = 10000, tipo } = r.data as {
      lat: number; lng: number; raioM?: number; tipo?: string;
    };

    // Carregar todos e filtrar client-side — evita índice composto no Firestore
    // Para volumes maiores, considerar geohash no futuro
    let q: any = db().collection('pois');
...
export const deletarPOI = onCall(
  { timeoutSeconds: 15, memory: '128MiB' },
  async (r: any) => {
    if (!r.auth) throw new HttpsError('unauthenticated', 'Auth necessária');
    const { poiId } = r.data as { poiId: string };
