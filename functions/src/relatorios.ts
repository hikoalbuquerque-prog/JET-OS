...
function gerarTextoPerdas(ocorrs: Ocorrencia[], label: string, semanal: boolean): string {
  ...
  const porFilial: Record<string, {
    total: number; roubos: number; recup: number;
    pat: number; bic: number; bat: number;
  }> = {};
  ocorrs.forEach(o => {
    const f = o.filial || o.cidade_inicial || 'Desconhecida';
    if (!porFilial[f]) porFilial[f] = { total:0, roubos:0, recup:0, pat:0, bic:0, bat:0 };
    porFilial[f].total++;
    if (o.tipo === 'Roubo' || o.tipo === 'Tentativa') porFilial[f].roubos++;
    if (/recuper/i.test(o.status || '') || o.tipo === 'Recuperacao') porFilial[f].recup++;
    const at = String(o.ativo_tipo || '').toLowerCase();
    if (at.includes('patinete')) porFilial[f].pat++;
    else if (at.includes('bicicleta')) porFilial[f].bic++;
    else if (at.includes('bateria'))   porFilial[f].bat++;
  });
  ...
  if (filiaisOrd.length) {
    ...
    filiaisOrd.slice(0, 8).forEach(([f, v]) => {
      ...
      if (ativos) txt += `  ${ativos}  |  🔴${v.roubos} roubos  ✅${v.recup} recup.\n`;
    });
    txt += `\n`;
  }
  ...
  if (filiaisOrd[0]) {
    txt += `${'─'.repeat(28)}\n`;
    txt += `*⚠️ Alertas*\n`;
    if (roubos.length > 0) txt += `• 🔴 *${roubos.length}* roubo(s) registrado(s) no período\n`;
    if (vand.length > 0) txt += `• 🟡 *${vand.length}* vandalismo(s)\n`;
    txt += `• 🔺 Maior volume: *${filiaisOrd[0][0]}* (${filiaisOrd[0][1].total})\n`;
    if (total === 0) txt += `• ✅ Nenhuma ocorrência no período\n`;
    txt += `\n`;
  }
...
export const relatorioPerdasDiario = onSchedule(
  { schedule: '0 7 * * 2-7', timeZone: 'America/Sao_Paulo', memory: '256MiB', timeoutSeconds: 300 },
  async () => { await enviarRelatorio('perdas', 'ontem'); }
);

// Perdas semanal — 7h toda segunda
export const relatorioPerdasSemanal = onSchedule(
  { schedule: '0 7 * * 1', timeZone: 'America/Sao_Paulo', memory: '256MiB', timeoutSeconds: 300 },
  async () => { await enviarRelatorio('perdas', 'semana'); }
);
