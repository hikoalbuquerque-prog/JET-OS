...
interface RelatorioGuard {
  ...
  comBO:            number;
  ocorrencias:      OcorrenciaRaw[];
  perdas:           PerdasResumo;
}

// ── PERDAS_ACUM (fallback hardcoded) ───────────────────────────────────────
const PERDAS_ACUM = {
...
// ── buscarDadosDinamicosFiliais (Supabase) ─────────────────────────────────
async function buscarDadosDinamicosFiliais(): Promise<Record<string, {
  ...
  try {
    const { data } = await sb()
      .from('guard_config')
      .select('*')
      .eq('id', 'controle_perdas')
      .single();
    if (!data) return {};
    const filiais: any[] = data.filiais || [];
...
function regiaoDaFilial(cidade: string): string {
  const c = cidade.toLowerCase();
  if (/bel[ée]m|minas|\(bh\)|fortaleza|cear|recife|pernambuc|aracaj|sergipe|salvador|bahia|vila velha|esp[íi]rito|natal|rg norte/.test(c)) return 'Norte';
  if (/sp capital|sp litoral|sp estado|campinas|s[ãa]o paulo/.test(c)) return 'Centro';
  if (/bras[íi]lia|distr|santa catarina|paran[áa]|londrina|rg sul|porto alegre|gramado|tramanda/.test(c)) return 'Sul';
  return '';
}
  ...
  try {
    ...
    for (let from = 0; ; from += PAGE) {
      const { data, error } = await sb()
        .from('ocorrencias')
        .select('ativo_tipo, cidade, criado_em')
        .eq('tipo', 'Perda')
        .range(from, from + PAGE - 1);
      if (error) { if (from === 0) return perdasFallback(); break; }
      if (!data) break;
      rows.push(...data);
    ...
    const fim   = dataRef.getTime() + 86400000;
    ...
    for (const r of rows) {
      ...
      if (!fil.has(cidade)) fil.set(cidade, { filial: cidade, regiao: regiaoDaFilial(cidade), resp: '', patins: 0, bikes: 0, baterias: 0, brpd: 0 });
...
function faixaSemana(d: Date): string {
  const dow    = d.getDay();
  const inicio = new Date(d);
  inicio.setDate(d.getDate() - (dow === 0 ? 6 : dow - 1));
  const fim    = new Date(inicio); fim.setDate(inicio.getDate() + 6);
  const fmt    = (x: Date) => x.toLocaleDateString('pt-BR', { day:'2-digit', month:'2-digit', timeZone:'America/Sao_Paulo' });
  return 'Semana ' + String(semanaISO(d)).padStart(2,'0') + '  ' + fmt(inicio) + ' - ' + fmt(fim);
}

function fmtDt(ts: string | null | undefined): string {
  if (!ts) return '';
  const d = new Date(ts);
...
function sanitizarFoto(url?: string): string | null {
  if (!url) return null;
...
function siglaResponsavel(cargo: string): string {
  const c = (cargo || '').toLowerCase();
  if (c.includes('seg') || c.includes('guard')) return 'SEG';
  if (c.includes('mot') || c.includes('driver')) return 'MOT';
  if (c.includes('gest')) return 'GST';
  if (c.includes('camp')) return 'CAM';
...
function labelMes(dataRef: Date): string {
...
function labelAcumulado(ocs: OcorrenciaRaw[]): string {
  if (!ocs.length) return 'Acumulado';
...
// ── GERAR RELATÓRIO ───────────────────────────────────────────────────────
async function gerarRelatorioGuard(dataStr?: string): Promise<RelatorioGuard> {
  let dataRef: Date;
  if (dataStr) {
  ...
  const startISO = dataRef.toISOString();
...
function buildMensagem(lang: string, r: RelatorioGuard, ocs24h: OcorrenciaRaw[], ocs7d: OcorrenciaRaw[]): string {
  const locale = ({ pt:'pt-BR', en:'en-US', es:'es-ES', ru:'ru-RU' } as Record<string,string>)[lang] ?? 'pt-BR';
  const dataRef = new Date(r.data + 'T12:00:00');
  const dataFmt = dataRef.toLocaleDateString(locale, {
    weekday:'long', day:'2-digit', month:'long', year:'numeric'
  });
  const fmt2 = (d: Date) => d.toLocaleDateString('pt-BR',
    { day:'2-digit', month:'2-digit', timeZone:'America/Sao_Paulo' });
  ...
  if (roubosAbertos > 0) txt += `• 🚨 ${tr(lang,'robbery')}: *${roubosAbertos}*\n`;
  if (vandAbertos   > 0) txt += `• 🟡 ${tr(lang,'vandalism')}: *${vandAbertos}*\n`;
  if (procurand     > 0) txt += `• ‼️ *${tr(lang,'wanted')}: ${procurand}*\n`;
  ...
  if (r.comBO) txt += `• ${tr(lang,'with_bo')}: *${r.comBO}*\n`;
  txt += `\n`;
  ...
  if (porCidadeOrd.length) {
    ...
    porCidadeOrd.slice(0, 6).forEach(([cidade, n]) => {
      const oc2 = ocs.filter(o => o.cidade === cidade);
  ...
  ocs.forEach(o => {
    const nome = o.registrado_por_nome || '?';
    if (!guardsCont[nome]) guardsCont[nome] = { n:0, cargo: o.cargo || '' };
    guardsCont[nome].n++;
  });
  ...
  if (procurand > 0)     alertas.push(`‼️ *${procurand} ${tr(lang,'alert_wanted')}*`);
  if (crit > 0)          alertas.push(`🚨 ${crit} ${tr(lang,'alert_critical')}`);
  if (roubosAbertos > 0) alertas.push(`🔴 ${roubosAbertos} ${tr(lang,'alert_rob_open')}`);
  if (roubos24h >= 3)    alertas.push(`🔺 ${tr(lang,'alert_multiple')}: ${roubos24h}`);
...
// ── GERAR PDF HTML ─────────────────────────────────────────────────────────
async function gerarPdfHtml(
  r: RelatorioGuard,
  ocs24h: OcorrenciaRaw[],
  ocs7d: OcorrenciaRaw[],
  ocsMes: OcorrenciaRaw[],
  ocsAcum: OcorrenciaRaw[],
  lang: string = 'pt'
): Promise<string> {
  ...
  const fora    = r.ocorrencias.filter(o => !o.lat).length;
  ...
  const vandComDano   = ocorrs.filter(o => o.tipo === 'Vandalismo' && (o.danoValor ?? 0) > 0).length;
  const danoValorTotal= ocorrs.filter(o => o.tipo === 'Vandalismo')
    .reduce((s: number, o) => s + (o.danoValor || 0), 0);
  const danoPctMedio  = (() => {
    const com = ocorrs.filter(o => o.tipo === 'Vandalismo' && (o.danoPct ?? 0) > 0);
    return com.length > 0
      ? Math.round(com.reduce((s: number, o) => s + (o.danoPct || 0), 0) / com.length) : 0;
  })();
  ...
  const porCidadeMap: Record<string, {
    total:number; roubos:number; vand:number; tent:number; recup:number;
    abertos:number; crit:number; fora:number; ocorrs:OcorrenciaRaw[];
  }> = {};
  r.ocorrencias.forEach(o => {
    const c = o.cidade || 'Desconhecida';
    if (!porCidadeMap[c]) porCidadeMap[c] = {total:0,roubos:0,vand:0,tent:0,recup:0,abertos:0,crit:0,fora:0,ocorrs:[]};
    const v = porCidadeMap[c];
    v.total++; v.ocorrs.push(o);
    if (o.tipo==='Roubo') v.roubos++;
    if (o.tipo==='Tentativa') v.tent++;
    if (o.tipo==='Vandalismo') v.vand++;
    if (/recuper/i.test(o.status||'')) v.recup++;
    if (o.status==='Aberto') v.abertos++;
    if (o.prioridade==='Alta'||o.prioridade==='Critica') v.crit++;
    if (!o.lat) v.fora++;
  });
  ...
  if (statusData.length && total > 0) {
    const W2=220; const H2=120; const cx=60; const cy=55; const r2=45;
    ...
    statusData.forEach(([s,n]) => {
      ...
      const x1 = cx + r2*Math.cos(ang); const y1 = cy + r2*Math.sin(ang);
      const x2 = cx + r2*Math.cos(ang+a2); const y2 = cy + r2*Math.sin(ang+a2);
  ...
  const alertas: string[] = [];
  if (procurand > 0) alertas.push(`<b>‼️ ${procurand} ativo(s) PROCURADO(S)</b> — requer ação imediata!`);
  if (crit > 0) alertas.push(`🚨 ${crit} ocorrência(s) crítica(s) no período`);
  if (abertos + apurac > 0) alertas.push(`⏳ ${abertos + apurac} pendente(s) de resolução`);
  if (roubos24h.length >= 3) alertas.push(`🔺 Múltiplos roubos nas últimas 24h: ${roubos24h.length}`);
  if (total > 0 && fora/total > 0.25) alertas.push(`📍 ${pct(fora,total)} das ocorrências fora de zona cadastrada`);
  if (cidadesOrd[0]?.[1].total/total > 0.5) alertas.push(`📌 Concentração: +50% em ${cidadesOrd[0][0]}`);

  const roubosRows24h = [...roubos24h, ...tent24h].sort((a,b) => {
    const da = new Date(a.criado_em).getTime() || 0;
    const db2= new Date(b.criado_em).getTime() || 0;
    return db2 - da;
  }).map(o => {
    ...
    const fotosHtml = [f1, f2].filter(Boolean).map((url, i) =>
    ...
    return `<tr><td>${fmtDt(o.criado_em)}</td>
  ...
  const vandRows24h = vand24h.sort((a,b) => {
    ...
    return `<tr><td>${fmtDt(o.criado_em)}</td>
      ...
      <td>${o.registrado_por_nome||'—'}</td>
      <td>${o.bo_numero||'—'}</td>
      <td>${(o.danoValor??0)>0?`R$${o.danoValor}`:(o.danoPct??0)>0?`${o.danoPct}%`:'—'}${btnFotos}</td></tr>`;
  }).join('');

  const chartData = (() => {
    const now = new Date();
    const dayLabels: string[] = []; const dayR: number[] = []; const dayT: number[] = []; const dayV: number[] = [];
    for (let i = 29; i >= 0; i--) {
      const d = new Date(now); d.setDate(d.getDate() - i); d.setHours(0,0,0,0);
      const next = new Date(d); next.setDate(next.getDate() + 1);
      dayLabels.push(d.toLocaleDateString('pt-BR',{day:'2-digit',month:'2-digit'}));
      dayR.push(ocsAcum.filter(o => { const t = new Date(o.criado_em); return o.tipo==='Roubo' && t>=d && t<next; }).length);
      dayT.push(ocsAcum.filter(o => { const t = new Date(o.criado_em); return o.tipo==='Tentativa' && t>=d && t<next; }).length);
      dayV.push(ocsAcum.filter(o => { const t = new Date(o.criado_em); return o.tipo==='Vandalismo' && t>=d && t<next; }).length);
    }
    const wkLabels: string[] = []; const wkR: number[] = []; const wkT: number[] = []; const wkV: number[] = [];
    for (let i = 11; i >= 0; i--) {
      ...
      wkR.push(ocsAcum.filter(o => { const t = new Date(o.criado_em); return o.tipo==='Roubo' && t>=ini && t<fim; }).length);
      wkT.push(ocsAcum.filter(o => { const t = new Date(o.criado_em); return o.tipo==='Tentativa' && t>=ini && t<fim; }).length);
      wkV.push(ocsAcum.filter(o => { const t = new Date(o.criado_em); return o.tipo==='Vandalismo' && t>=ini && t<fim; }).length);
    }
    const moLabels: string[] = []; const moR: number[] = []; const moT: number[] = []; const moV: number[] = [];
    const yr = now.getFullYear();
    const meses = ['Jan','Fev','Mar','Abr','Mai','Jun','Jul','Ago','Set','Out','Nov','Dez'];
    for (let m = 0; m < 12; m++) {
      const ini = new Date(yr, m, 1); const fim = new Date(yr, m+1, 1);
      moLabels.push(meses[m]);
      moR.push(ocsAcum.filter(o => { const t = new Date(o.criado_em); return o.tipo==='Roubo' && t>=ini && t<fim; }).length);
      moT.push(ocsAcum.filter(o => { const t = new Date(o.criado_em); return o.tipo==='Tentativa' && t>=ini && t<fim; }).length);
      moV.push(ocsAcum.filter(o => { const t = new Date(o.criado_em); return o.tipo==='Vandalismo' && t>=ini && t<fim; }).length);
    }
  ...
  const perdasRows = filiaisOrdenadas
    .map((f,i) => {
      const d     = dadosDinamicos[f.filial] || {} as any;
  ...
  cidadesOrd.forEach(([cidade, v]) => {
    ...
    v.ocorrs.sort((a,b) => {
      const order: Record<string,number> = {'Aberto':0,'Em apuração':1,'Em apuracao':1,'Recuperado':2,'Encerrado':3};
      ...
      if (isCrit) apendice += `<span style="background:#c0392b;color:#fff;padding:2px 8px;border-radius:12px;font-size:9px;font-weight:700">CRÍTICO</span>`;
      if (isProc) apendice += `<span style="background:#9b59b6;color:#fff;padding:2px 8px;border-radius:12px;font-size:9px;font-weight:700">‼️ PROCURADO</span>`;
      apendice += `<span style="margin-left:auto;color:#999;font-size:9px">${fmtDt(o.criado_em)}</span>`;
      apendice += `</div>`;

      const sigla = siglaResponsavel(o.cargo || '');
      const infos: string[] = [];
      if (o.ativo_tipo||o.asset_id) infos.push(`Ativo: <b>${[o.ativo_tipo,o.asset_id].filter(Boolean).join(' ')}</b>`);
      if (o.registrado_por_nome) infos.push(`Guard: <b>${o.registrado_por_nome}</b> <span style="background:#1a1a2e;color:#fff;padding:1px 5px;border-radius:4px;font-size:8px;font-weight:700">${sigla}</span>`);
      if (o.turno) infos.push(`Turno: ${o.turno}`);
      if (o.prioridade) infos.push(`Prioridade: <b style="color:${isCrit?'#c0392b':'inherit'}">${o.prioridade}</b>`);
      if (o.bo_numero) infos.push(`BO: <b>${o.bo_numero}</b>`);
      if (infos.length) apendice += `<div class="ocorr-info">${infos.join('  ·  ')}</div>`;

      if (o.bairro||o.endereco) {
        apendice += `<div class="ocorr-local">📍 ${[o.bairro,o.endereco].filter(Boolean).join(' — ')}</div>`;
      }
      if (o.descricao) {
...
<div class="alerta-box">
  <div class="alerta-row">
    ...
    <div class="ab-card"><div class="ab-val" style="color:#f97316">${ocorrs.filter((o:any)=>o.tipo==='Tentativa'&&!/recuper|encerr/i.test(o.status||'')).length}</div><div class="ab-lbl"><span data-lk="attempt">${tr(lang,'attempt')}</span> <span data-lk="open_lbl">${tr(lang,'open_lbl')}</span></div></div>
...
<div class="pg-kpis">
  ...
  <div class="pk" style="border-top-color:#f97316"><div class="pv">${ocorrs.filter((o:any)=>o.tipo==='Tentativa').length}</div><div class="pl">🟠 <span data-lk="attempt">${tr(lang,'attempt')}</span></div></div>
...
${(() => {
  const semanas = 10;
  const entries: {label:string; roubos:number; tent:number}[] = [];
  ...
  const W=820; const PL=8; const PR=8; const PT=30; const PB=36;
...
<div class="section">
  ...
  <table>
    ...
    <tbody>${cidadesOrd.map(([c,v]) =>
      `<tr><td><b>${c}</b></td>
      <td class="n" style="color:#c0392b;font-weight:700">${v.roubos||0}</td>
      <td class="n" style="color:#e67e22">${ocorrs.filter((o:any)=>o.tipo==='Tentativa'&&o.cidade===c).length}</td>
      <td class="n" style="color:#c0392b">${v.abertos||0}</td>
      <td class="n" style="color:#e74c3c">${v.crit||0}</td></tr>`
    ).join('')}</tbody>
  </table>
</div>`:''}
...
<div class="section">
  ...
  <div class="roubos-box" style="border-color:#f0ad4e;border-left-color:#b7950b;background:#fffbf0">
    <div class="roubos-kpis">
      ...
      <div class="rkpi"><div class="rkv" style="color:#e67e22">${ocs7d.filter((o:any)=>o.tipo==='Vandalismo').length}</div><div class="rkl">🟡 <span data-lk="vandalism">${tr(lang,'vandalism')}</span><br><b style="font-size:8px;color:#999" data-lk="pdf_7d">${tr(lang,'pdf_7d')}</b></div></div>
...
${(() => {
  const semanas = 10;
  const entries: {label:string; vand:number}[] = [];
  for (let i = semanas-1; i >= 0; i--) {
    const ini = new Date(Date.now() - (i+1)*7*86400000);
    const fim = new Date(Date.now() - i*7*86400000);
    const semOcs = r.ocorrencias.filter(o => {
  ...
  const W=820; const PL=8; const PR=24; const PT=30; const PB=36;
  const barW = Math.floor((W-PL-PR)/entries.length);
...
<div style="display:flex;gap:16px;padding:0 32px 16px;flex-wrap:wrap">
  <div style="flex:1;min-width:200px;background:#f8f9fa;border-radius:8px;padding:12px">
    ...
    ${[
      ['🛴 Patinetes', ocorrs.filter((o:any)=>o.tipo==='Vandalismo'&&/patinete/i.test(o.ativo_tipo||'')).length, '#f59e0b'],
      ['🚲 Bicicletas', ocorrs.filter((o:any)=>o.tipo==='Vandalismo'&&/bicicleta|bike/i.test(o.ativo_tipo||'')).length, '#3b82f6'],
      ['🔋 Baterias',  ocorrs.filter((o:any)=>o.tipo==='Vandalismo'&&/bateria/i.test(o.ativo_tipo||'')).length, '#a78bfa'],
    ].filter(([,n])=>(n as number)>0).map(([l,n,c]) => {
      const bw = vand > 0 ? Math.max(4,Math.round((n as number)/vand*200)) : 0;
...
${(() => {
  const vandCidades: Record<string,number> = {};
  ocs7d.filter((o:any)=>o.tipo==='Vandalismo').forEach((o:any)=>{
    const c = (o.cidade||'Desconhecida').split(' ')[0].slice(0,12);
  ...
  if(!entries.length) return '<div class="section" style="color:#999;font-size:11px;padding:8px 32px">Sem dados de vandalismo nos últimos 7 dias</div>';
  const maxV = entries[0][1]||1;
  const W=820; const PL=110; const PR=20; const PT=36; const PB=28; const barH=18; const gap=5;
...
<div class="section">
  ...
  <table>
    ...
    <tbody>${cidadesOrd.filter(([,v])=>v.vand>0).map(([c,v]) =>
      `<tr><td><b>${c}</b></td>
...
<div class="alertas">
  ...
  <ul>${alertas.map((a:string)=>`<li>• ${a}</li>`).join('')}</ul>
</div>`:''}
...
async function guardDiario() {
  ...
  const r = await gerarRelatorioGuard(date);

  const agora = new Date();
  const ini24 = new Date(agora.getTime() - 24*3600000).toISOString();
  const ini7d = new Date(agora.getTime() - 7*24*3600000).toISOString();
  const iniMes = new Date(agora.getFullYear(), agora.getMonth(), 1).toISOString();
  const iniAno = new Date(agora.getFullYear(), 0, 1).toISOString();
  const agoraISO = agora.toISOString();

  const [res24, res7d, resMes, resAcum] = await Promise.all([
    sb().from('ocorrencias').select('*').gte('criado_em', ini24).lte('criado_em', agoraISO).order('criado_em', { ascending: false }),
    sb().from('ocorrencias').select('*').gte('criado_em', ini7d).lte('criado_em', agoraISO).order('criado_em', { ascending: false }),
    sb().from('ocorrencias').select('*').gte('criado_em', iniMes).lte('criado_em', agoraISO).order('criado_em', { ascending: false }),
    sb().from('ocorrencias').select('*').gte('criado_em', iniAno).lte('criado_em', agoraISO).order('criado_em', { ascending: false }),
  ]);

  const ocs24h  = (res24.data ?? []) as OcorrenciaRaw[];
  const ocs7d   = (res7d.data ?? []) as OcorrenciaRaw[];
  const ocsMes  = (resMes.data ?? []) as OcorrenciaRaw[];
  const ocsAcum = (resAcum.data ?? []) as OcorrenciaRaw[];

  const lang = 'pt';
  const mensagem = buildMensagem(lang, r, ocs24h, ocs7d);
  ...
  if (cfg.guard_chat_id) {
    await sendTelegramText(cfg.guard_chat_id, mensagem, cfg.guard_thread_id);
    await sendTelegramDocument(
      cfg.guard_chat_id,
      new TextEncoder().encode(html),
      filename,
      `📎 JET Guard • ${tr(lang,'pdf_caption')} • ${r.data}`,
      cfg.guard_thread_id,
    );
  }

  return { ok: true, totalOcorrencias: r.totalOcorrencias, data: date, tipo: 'guard' };
}
...
async function guardSemanal() {
  ...
  const [{ data: rows }, { data: prevRows }] = await Promise.all([
    sb()
      .from("ocorrencias")
      .select("*")
      .gte("criado_em", `${range.start}T00:00:00`)
      .lte("criado_em", `${range.end}T23:59:59.999`),
    sb()
      .from("ocorrencias")
      .select("*")
      .gte("criado_em", `${prevRange.start}T00:00:00`)
      .lte("criado_em", `${prevRange.end}T23:59:59.999`),
  ]);

  const occ = (rows ?? []) as OcorrenciaRaw[];
  const prevOcc = (prevRows ?? []) as OcorrenciaRaw[];

  // Build RelatorioGuard for the week range
  const porTipo: Record<string,number> = {};
  const porStatus: Record<string,number> = {};
  const porCidade: Record<string,number> = {};
  const porTurno: Record<string,number> = {};
  ...
  const dataRefDate = new Date(range.start + 'T12:00:00');
  const perdas = await buscarPerdasSupabase(dataRefDate);

  const r: RelatorioGuard = {
    data: `${range.start} → ${range.end}`,
    semana: faixaSemana(dataRefDate),
    totalOcorrencias: occ.length,
    porTipo, porStatus, porCidade, porTurno,
    altaPrioridade, comBO,
    ocorrencias: occ,
    perdas,
  ...
  // Previous week aggregate for comparison
  const prevAgg = { totalOcorrencias: prevOcc.length };
  const delta = prevAgg.totalOcorrencias > 0
    ? Math.round(((r.totalOcorrencias - prevAgg.totalOcorrencias) / prevAgg.totalOcorrencias) * 100)
    : 0;
  const deltaStr = delta > 0 ? `+${delta}%` : `${delta}%`;

  // Fetch periods for full report
  const agora = new Date();
  const ini24 = new Date(agora.getTime() - 24*3600000).toISOString();
  const ini7d = new Date(agora.getTime() - 7*24*3600000).toISOString();
  const iniMes = new Date(agora.getFullYear(), agora.getMonth(), 1).toISOString();
  ...
  const lang = 'pt';
  let md = buildMensagem(lang, r, ocs24h, ocs7d);
  md += `\n*Comparativo semana anterior:* ${prevAgg.totalOcorrencias} → ${r.totalOcorrencias} (${deltaStr})\n`;
...
async function perdasDiario() {
  ...
  const { data: rows, error } = await sb()
    ...
    .select("*")
    .eq("tipo", "Perda")
    .gte("criado_em", `${date}T00:00:00`)
    .lt("criado_em", `${date}T23:59:59.999`);
  if (error) throw error;
  const occ = (rows ?? []) as OcorrenciaRaw[];

  const perdasAgg = await buscarPerdasSupabase(new Date(date + 'T12:00:00'));

  let md = `📉 *Relatório de Perdas — ${date}*\n`;
  md += `Total 24h: *${occ.length}* perdas\n`;
  md += `🛴 ${perdasAgg.totalPatins} patins | 🚲 ${perdasAgg.totalBikes} bikes | 🔋 ${perdasAgg.totalBaterias} baterias\n`;
  md += `BRPD acumulado: *${perdasAgg.totalBRPD}*\n`;
  md += `Últimos 7d: *${perdasAgg.perdas7d}* | 24h: *${perdasAgg.perdas24h}*\n`;
...
async function perdasSemanal() {
  ...
  const perdasAgg = await buscarPerdasSupabase(new Date(range.end + 'T12:00:00'));

  let md = `📉 *Relatório de Perdas Semanal — ${range.start} → ${range.end}*\n`;
  md += `Total semana: *${occ.length}* perdas\n`;
  md += `🛴 ${perdasAgg.totalPatins} patins | 🚲 ${perdasAgg.totalBikes} bikes | 🔋 ${perdasAgg.totalBaterias} baterias\n`;
  md += `BRPD acumulado: *${perdasAgg.totalBRPD}*\n`;
  md += `Últimos 7d: *${perdasAgg.perdas7d}* | 24h: *${perdasAgg.perdas24h}*\n`;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.error(e);
    return json({ error: e instanceof Error ? e.message : JSON.stringify(e) }, 500);
  }
});
